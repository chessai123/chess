...
class chessBoard:
    ...
    def check_if_legal(self):
        if self.check_if_promotion():
            # promote to a queen
            Nf3 = chess.Move(
                from_square=self.from_position,
                to_square=self.to_position,
    ...
    def move_piece(self, square):
        if self.to_position is None and self.from_position is not None:
    ...
    def run_game_playerVsAI(self):
        ...
        while True:
            if self.turn % 2 == 0:
                ...
                # Process events even during AI turn to keep window responsive
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        sys.exit()
    ...
    def run_game_playerVsPlayer(self):
        ...
        while True:
            if self.turn % 2 == 0:
                ...
                pygame.display.set_caption("AlfaGeir - Black to move")
    ...
    def cmd_choices(self):
        while 1:
            ...
            if command[0] == "1":
                self.run_game_playerVsAI()
            elif command[0] == "2":
                self.run_game_AIvAI()
            elif command[0] == "3":
                self.run_game_playerVsPlayer()
            elif command[0] == "quit" or command[0] == "exit":
                break


def main():
    """Entry point for the AlfaGeir chess game."""
